...
export const JobCard = ({ job }: { job: Job }) => {
  ...
  return (
    <div
      ...
      /* 
         FIX: Changed w-[250px] to w-full. 
         Now the grid handles the spacing and it will be equal on all sides.
      */
    ...
    >
      {/* 1. HEADER (SO#) */}
      <div className={`px-[18px] pt-[18px] pb-[10px] transition-colors duration-500 ${isHovered ? "bg-[#555555]" : "bg-white"}`}>
        <span className={`text-[14px] leading-[22px] font-normal uppercase transition-colors duration-500 block ${isHovered ? "text-white" : "text-[#006E74]"}`}>
          {job.id}
        </span>
      </div>
      ...
      <div className="flex-1 relative bg-white">
        ...
        <div className={`absolute inset-0 px-[18px] pb-[18px] transition-opacity duration-500 ease-in-out ${isHovered ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
          ...
          <div className="absolute bottom-[18px] left-[18px] right-[18px] flex items-center gap-2">
            <span className="bg-[#ECECE1] px-2.5 py-1 rounded-full text-[11px] text-[#231F20] font-medium whitespace-nowrap leading-none">Java Script</span>
            <span className="bg-[#ECECE1] px-2.5 py-1 rounded-full text-[11px] text-[#231F20] font-medium whitespace-nowrap leading-none">Python</span>
        ...
        {/* --- HOVER VIEW CONTENT --- */}
        <div className={`absolute inset-0 px-[18px] pb-[18px] pt-[8px] bg-white transition-opacity duration-500 ease-in-out ${isHovered ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
          <p className="text-[11.5px] leading-[17px] text-[#231F20] font-normal line-clamp-6">
            {job.description}
          </p>
          <button className="absolute bottom-[18px] left-[18px] text-[#0097AC] font-bold text-[12px] flex items-center gap-1 hover:text-black">
