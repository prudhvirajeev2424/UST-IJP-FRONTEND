...
export function UpdateResumeDrawer({
  ...
  // rect was previously computed to align the drawer with the modal. It's unused at the moment,
  // so remove the state to avoid a TS6133 'declared but its value is never read' error.
  const touchStartRef = useRef<number | null>(null);

  // (Position tracking removed) — drawer currently uses fixed placement. If alignment is
  // required later we can reintroduce a rect state and compute it here.
