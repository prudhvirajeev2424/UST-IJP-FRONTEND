...
const Navbar = ({ role, onNavigate, activePage = "Home" }: NavbarProps) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-white h-16 shadow-[0_1px_4px_rgba(0,0,0,0.08)]">
      <div className="relative max-w-7xl mx-auto h-full px-4 md:px-6">
        ...
        {/* MOBILE NAV */}
        {mobileOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 mt-2 mx-4 rounded-xl border bg-white shadow-lg">
            {links.map((link) => (
              <button
                key={link}
                onClick={() => handleNavClick(link)}
