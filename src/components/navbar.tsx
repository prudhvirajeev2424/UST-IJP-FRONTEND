...
import { ActiveRoleContext } from "../pages/landing_page";
...
const Navbar = ({ role = "Employee" }: NavbarProps) => {
  ...
  const { setActivePage } = useContext(ActiveRoleContext);
      ...
      <div className="max-w-7xl mx-auto w-full px-6 py-[14px]">
        <div className="flex items-center justify-between">
              ...
              {links.map((link) => (
                <div key={link} className="relative">
                  <button
                    ...
                    onClick={() => {
                      setActive(link);
                      if (setActivePage) setActivePage(link);
                    }}
          ...
          <div className="flex-shrink-0 flex items-center gap-4 relative">
            {role !== "WFM" && <Mail size={24} className="text-gray-700" />}
