...
const ProfileHomeList: React.FC<ProfileHomeListProps> = ({ profiles }) => {
  useEffect(() => {
    console.log(
      "ProfileHomeList mounted with",
      profiles?.length ?? 0,
      "profiles"
    );
  }, [profiles]);
 
  const handleRowClick = (soNumber: string, idx: number) => {
    // If this is the first row, request the detailed TP Applications view
    // so the app shows the full TP_Applications layout for the profile.
    if (idx === 0) {
    ...
    // Otherwise, open the regular Applications list view
    window.dispatchEvent(
      new CustomEvent("navigate", {
        detail: {
          view: "Applications",
      ...
      <table className="w-full">
        ...
        <tbody>
          {profiles.map((p, idx) => {
            ...
            const displayScore =
              scoreNum !== null ? `${scoreNum}%` : p.matchScore;
            ...
            return (
              <tr
                ...
                onClick={() => handleRowClick(p.soNumber ?? String(idx), idx)}
              ...
              >
                ...
                <td
                  className="py-4 px-3 font-rubik "
                  style={{
                    fontSize: "18px",
                    lineHeight: "22px",
                    letterSpacing: "0px",
                ...
                <td
                  className="py-4 px-2 font-rubik"
                  style={{
                    fontSize: "18px",
                    lineHeight: "22px",
                    letterSpacing: "0px",
