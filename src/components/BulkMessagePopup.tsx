...
import uploadIcon from "../assets/Icon feather-upload-cloud.svg";
...
function showToast(
  ...
  if (!toastContainer.querySelector(".toast-container")) {
    ...
    wrapper.style.cssText =
      "position: fixed; left: 50%; top: 98px; transform: translateX(-50%); z-index: 9999;";
...
const BulkMessagePopup: React.FC<BulkMessagePopupProps> = ({ onClose }) => {
        ...
        <div className="fixed inset-0 z-50 flex h-[1080px] w-[1920px]  items-center justify-center bg-[#231F20] bg-opacity-70">
          <div className="relative h-[580px] w-[601px] rounded-[10px] bg-[#FFFFFF] shadow-xl">
            ...
            <div className="flex h-[70px] w-[601px] items-center justify-between rounded-t-[10px] bg-[#F2F7F8] px-[32px] py-[24px]">
              ...
              <button
                className="flex h-[24px] w-[24px] cursor-pointer items-center justify-center border-none bg-transparent text-[20px] leading-none text-[#7A7480] hover:text-[#231F20]"
                onClick={() => {
                  setIsModalOpen(false);
                  onClose?.();
                }}
...
Andrea Stephen"
              ...
              <div>
                ...
                <div className="flex h-[126px] w-[521px] cursor-pointer flex-col items-center justify-center rounded-[10px] border border-dashed border-[#D7E0E3] bg-[#F7F9FA] text-[13px] text-[#7A7480] transition-colors ">
            ...
            <div className="flex justify-center gap-[16px] border-[#E5E5E5] px-[32px] py-[23px]">
              <button
                ...
                className="h-[49px] w-[90px] cursor-pointer rounded-[4px] border border-[#231F20] bg-white text-[16px] font-normal text-[#231F20] transition-colors hover:bg-[#020101] hover:text-white"
              >
                Cancel
              </button>
              <button
                onClick={handleSend}
                className="h-[49px] w-[78px] cursor-pointer rounded-[4px] border-none bg-[#006E74] text-[16px] font-normal text-white transition-colors hover:bg-[#020101]"
              >
                Send
              </button>
            </div>
