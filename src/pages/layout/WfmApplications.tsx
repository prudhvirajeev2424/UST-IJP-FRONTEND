  ...
  const filteredApplications = useMemo(() => {
    ...
    if (key.includes("pending"))
      return mockApplications.filter((a) =>
        a.action.toLowerCase().includes("pending")
      );
    ...
    if (key.includes("allocated"))
      return mockApplications.filter((a) =>
        a.action.toLowerCase().includes("allocat")
      );
    ...
    if (
      ...
      return mockApplications.filter(
        (a) =>
          a.action.toLowerCase().includes("shortlist") ||
          a.action.toLowerCase().includes("selected") ||
          a.action.toLowerCase().includes("accepted")
      );

    // Default: match by substring
    return mockApplications.filter((a) =>
      a.action.toLowerCase().includes(key.split(" ")[0])
    );
  }, [activeTab]);
