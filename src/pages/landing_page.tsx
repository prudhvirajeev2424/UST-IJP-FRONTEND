...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        // Set the active role based on selection
        setActiveRole(role);
      }, ZOOM_MS + FADE_MS);
    } catch (err: any) {
      console.error(err);
      ...
      <div className="h-screen w-screen flex items-center justify-center bg-gray-100">
        <div className="bg-white rounded-2xl shadow-2xl p-10 text-center">
          ...
          <p className="text-lg text-gray-600 mb-6">
            You are logged in as: <span className="font-semibold text-[#008080]">{activeRole}</span>
          </p>
          <button
            onClick={() => {
              setActiveRole(null);
  ...
  return (
    <div className="h-screen w-screen flex items-center justify-center bg-[#008080] overflow-hidden">
      {/* Component-level styles for animations */}
      <style>{`
      ...
      {/* Centered Login Card */}
      <div className={`login-card bg-white rounded-2xl shadow-2xl p-10 w-full max-w-md mx-4 ${zoomOut ? 'zoom-out' : ''}`}>
        <div className={`transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0 fade-in-up' : 'opacity-0 -translate-y-4'}`}>
          {/* Logo */}
          ...
          {/* Error Message */}
          {error && (
            <div className={`mb-4 text-sm text-red-600 text-center ${errorShake ? 'shake' : ''}`}>
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#008080] ${errorShake ? 'shake' : ''}`}
