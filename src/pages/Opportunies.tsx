      ...
      <div className="max-w-[1650px] mx-auto px-[35px] pt-[30px] pb-[50px]">
            ...
            <div className="flex gap-1.5">
              {["All", "My Opportunities", "Shortlisted", "Applied"].map((tab) => (
                <button
                  ...
                  className={`px-4 h-[30px] rounded-full text-[11px] font-medium transition-all flex items-center justify-center whitespace-nowrap ${
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search"
                  className="pl-9 pr-4 h-[36px] w-[180px] border border-[#D7E0E3] rounded-md outline-none text-[12px] bg-white focus:border-[#006E74]"
                />
              </div>
              <div className="w-8 h-8 bg-white border border-[#D7E0E3] rounded-md flex items-center justify-center cursor-pointer hover:bg-gray-50">
                <img src={sortImg} alt="S" className="w-4" />
              </div>
              <div className="w-8 h-8 bg-white border border-[#D7E0E3] rounded-md flex items-center justify-center cursor-pointer hover:bg-gray-50">
                <img src={excelImg} alt="E" className="w-4" />
              </div>
              <div className="flex bg-white border border-[#D7E0E3] rounded-md h-8 overflow-hidden">
                <button
                  className={`w-8 flex items-center justify-center border-r border-[#D7E0E3] ${viewMode === "grid" ? "bg-[#F8FDFE]" : ""}`}
                  onClick={() => setViewMode("grid")}
                >
                  <img src={viewMode === "grid" ? gridDark : gridLight} alt="G" className="w-[14px]" />
                </button>
                <button
                  className={`w-8 flex items-center justify-center ${viewMode === "list" ? "bg-[#F8FDFE]" : ""}`}
                  onClick={() => setViewMode("list")}
                >
                  <img src={viewMode === "list" ? listDark : listLight} alt="L" className="w-[14px]" />
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-[30px] items-start">
          ...
          <div className="flex-1 min-w-0">
            {viewMode === "grid" ? (
              /* 
                 FIX: Uniform gap-5 ensures equal spacing on all sides.
                 The grid-cols are defined strictly so cards are perfectly sized.
              */
